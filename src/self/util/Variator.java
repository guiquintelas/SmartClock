...
public class Variator {
	...
	public void fadeInSin(double inicio, double max, int ticks) {
		...
		this.ticks = ticks;
		aux = 0.0;
		var = (200.0/(double)ticks) * ((max - inicio) * Math.sin(Math.toRadians(aux)))/MEIO_SIN;	
		isFadeIn = true;
		isSin = true;
		fadeInMax = max;
	}
	...
	public void fadeOutSin(double inicio, double min, int ticks) {
		...
		this.ticks = ticks;
		aux = 180.0;
		var = (200.0/(double)ticks) * ((inicio - min) * Math.sin(Math.toRadians(aux)))/MEIO_SIN;
	...
	public void update() {
		...
		checaVariando();
		checaEsperando();
		if (!isFadeIn && !isOscilando && !isFadeOut && !isEsperando) execeutarAcao();
	}
