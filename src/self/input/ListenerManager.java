...
public class ListenerManager implements java.awt.event.MouseListener, java.awt.event.MouseMotionListener, java.awt.event.MouseWheelListener, java.awt.event.KeyListener {
	...
	public static final int MOUSE_EXITED = 9;
	...
	private static ArrayList<MouseListener> mouseExited = new ArrayList<MouseListener>();
	private static ArrayList<MouseWheelListener> mouseWheel = new ArrayList<MouseWheelListener>();
	...
	public void init(JFrame janela) {
		...
		janela.getContentPane().addMouseWheelListener(this);
		//janela precisa estar focada para receber key strokes
		janela.addKeyListener(this);
	}
	
	static {
		System.out.println("Carregando Input Listeners...");
		...
		mapMouse.put(MOUSE_EXITED, mouseExited);
	...
	public static synchronized void addListener(MouseWheelListener ml) {
		if (mouseWheel.contains(ml)) return;
		mouseWheel.add(ml);
	}

	public static synchronized void removeListener(MouseWheelListener ml) {
		mouseWheel.remove(ml);
	}
	...
	public void mouseMoved(MouseEvent e) {
		...
		for (int i = 0; i < mouseMoved.size(); i++) {
			mouseMoved.get(i).acao(e);
		}
	...
	public void mouseExited(MouseEvent e) {
		...
		for (MouseListener md : mouseExited) {
			md.acao(e);
		}
	}

	public void mouseWheelMoved(MouseWheelEvent e) {
		for (MouseWheelListener md : mouseWheel) {
			md.acao(e);
		}
	}
